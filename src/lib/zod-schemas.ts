...
// Matching and Profil <-> Offre engine schema
export const matchItemSchema = z.object({
  requirement: z.string(),
  status: z.enum([
    "demonstrated",         // ✅ compétence démontrée
    "weakly_demonstrated",  // ⚠️ présente mais mal démontrée
    "to_confirm",           // ❓ information à confirmer
    "absent",               // ❌ réellement absente
  ]),
  evidence: z.string().optional(),
  suggestion: z.string().optional(), // uniquement si weakly_demonstrated
});
...
// Interactive AI Questions Schema
...
export const actionableBlockerSchema = z.object({
  issue: z.string().describe("Point bloquant précis ou lacune détectée"),
  location: z.string().describe("Section ou point ciblé (ex: Expérience TechScale, Compétences clés)"),
  why: z.string().describe("Impact négatif sur le filtrage ATS ou l'évaluation du recruteur"),
  fix: z.string().describe("Action corrective exacte recommandée"),
  severity: z.enum(["critical", "warning", "tip"]).describe("Niveau de priorité"),
});
...
export const resumeScoreSchema = z.object({
  ...
  blockers: z.array(actionableBlockerSchema).default([]),
...
export type MatchItemType = z.infer<typeof matchItemSchema>;
export type MatchAnalysisType = z.infer<typeof matchAnalysisSchema>;
export type PendingQuestionType = z.infer<typeof pendingQuestionSchema>;
export type ActionableBlockerType = z.infer<typeof actionableBlockerSchema>;