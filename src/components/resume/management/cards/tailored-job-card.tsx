...
import { BriefcaseIcon, FileSearch, Sparkles as SparklesIcon } from "lucide-react";
import { formatJobListing } from "@/utils/actions/jobs/ai";
import { ApplicationStatusBadge } from "../application-status-badge";
import { MatchAnalysisCard } from "../match-analysis-card";
import { PendingQuestionsCard } from "../pending-questions-card";
...
export function TailoredJobCard({ 
  ...
  const handleCreateJobWithAI = async () => {
    ...
    try {
      ...
      const LOCAL_STORAGE_KEY = 'easywork-api-keys';
...
export function TailoredJobAccordion({
  ...
  return (
    <AccordionItem value="job" className="mb-4 backdrop-blur-xl rounded-lg shadow-lg bg-white border-pink-500/40 border-2 overflow-hidden">
      <div className="px-4 py-1 flex items-center justify-between gap-2">
        <AccordionTrigger className="hover:no-underline group flex-1 py-3">
          <div className="flex items-center gap-3">
            <div className={cn("p-1.5 rounded-md transition-transform duration-300 group-data-[state=open]:scale-105", "bg-pink-100/80")}>
              <BriefcaseIcon className={cn("h-4 w-4", "text-pink-600")} />
            </div>
            <div className="flex flex-col items-start text-left">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-pink-700">Candidature</span>
                <span className="text-xs text-muted-foreground">•</span>
                <span className={cn("text-sm font-semibold", "text-gray-900")}>{title}</span>
              </div>
              {company && (
                <span className="text-xs text-pink-700/80 font-medium">{company}</span>
              )}
            </div>
          </div>
        </AccordionTrigger>

        {/* Status Badge clickable right in header */}
      ...
      <AccordionContent className="px-4 pb-4">
        <Tabs defaultValue="job-details" className="w-full mt-2">
          <TabsList className="w-full grid grid-cols-3 bg-pink-50/60 p-1 border border-pink-100 rounded-lg">
            <TabsTrigger value="job-details" className="text-xs font-semibold data-[state=active]:bg-white data-[state=active]:shadow-xs">
              Offre d&apos;emploi
            </TabsTrigger>
            <TabsTrigger value="match-engine" className="text-xs font-semibold data-[state=active]:bg-white data-[state=active]:shadow-xs">
              Adéquation Offre ↔ Profil
            </TabsTrigger>
            <TabsTrigger value="ai-questions" className="text-xs font-semibold data-[state=active]:bg-white data-[state=active]:shadow-xs">
              Questions IA {resume.pending_questions && resume.pending_questions.length > 0 ? `(${resume.pending_questions.length})` : ''}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="job-details" className="pt-3">
            <TailoredJobCard 
            ...
            {job && (
              <div className="flex justify-end mt-2">
                <Button
                  ...
                  disabled={isDeleting}
                  className="text-gray-400 hover:text-red-500 hover:bg-red-50/50 rounded-lg gap-2 text-xs"
                >
                  {isDeleting ? (
                  ...
                  Supprimer l&apos;offre liée
                </Button>
              </div>
            )}
          </TabsContent>

          <TabsContent value="match-engine" className="pt-3">
            <MatchAnalysisCard matchAnalysis={resume.match_analysis} />
          </TabsContent>

          <TabsContent value="ai-questions" className="pt-3">
            {resume.pending_questions && resume.pending_questions.length > 0 ? (
              <PendingQuestionsCard
                resumeId={resume.id}
                questions={resume.pending_questions}
              />
            ) : (
              <div className="p-6 text-center bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
                <p className="text-xs text-gray-500 font-medium">Aucune question de clarification en attente pour cette candidature.</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
