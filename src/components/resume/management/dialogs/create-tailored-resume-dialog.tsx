...
import { tailorResumeToJob, formatJobListing, analyzeJobMatch, generateClarificationQuestions } from "@/utils/actions/jobs/ai";
...
export function CreateTailoredResumeDialog({ children, baseResumes, profile }: CreateTailoredResumeDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      try {
        // Run match analysis & interactive clarification questions
        matchAnalysis = await analyzeJobMatch(baseResume, formattedJobListing, jobEntry.id, {
          model: selectedModel || '',
          apiKeys
        });

        pendingQuestions = await generateClarificationQuestions(matchAnalysis, baseResume, formattedJobListing, {
          model: selectedModel || '',
          apiKeys
        });
      ...
      // 5. Create the tailored resume with job reference, candidature status, and match analysis
      const resume = await createTailoredResume(
        baseResume,
        jobEntry.id,
